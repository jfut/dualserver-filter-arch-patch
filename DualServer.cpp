...
const char sVersion[] = "Dual DHCP DNS Server Version 7.12 Windows Build 7023";
...
MYWORD gdnmess(data5 *req, MYBYTE sockInd)
{
	...
	if (findEntry(currentInd, IP2String(req->cname, iip)))
		return req->bytes;
		...
		if (req->dnType == DNTYPE_CHILDZONE)
		{
			...
			if (req->remote.sin_addr.s_addr != cfig.dnsRoutes[zoneDNS].dns[cfig.dnsRoutes[zoneDNS].currentDNS])
			{
				...
				if (errno || nRet <= 0)
				{
					if (verbatim || cfig.dnsLogLevel)
					{
						sprintf(logBuff, "Error Forwarding UDP DNS Message to Conditional Forwarder %s", IP2String(tempbuff, req->addr.sin_addr.s_addr));
						logDNSMess(req, logBuff, 1);
						addRRNone(req);
				...
				else
				{
					if (verbatim || cfig.dnsLogLevel >= 2)
					{
						sprintf(logBuff, "%s forwarded to Conditional Forwarder %s", strquery(req), IP2String(tempbuff, cfig.dnsRoutes[zoneDNS].dns[cfig.dnsRoutes[zoneDNS].currentDNS]));
						logDNSMess(req, logBuff, 2);
					}
				}
			}
	...
	if (req->dnType != DNTYPE_CHILDZONE)
	{
		...
		if (req->remote.sin_addr.s_addr != network.dns[network.currentDNS])
		{
			...
			if (errno || nRet <= 0)
			{
				if (verbatim || cfig.dnsLogLevel)
				{
					sprintf(logBuff, "Error forwarding UDP DNS Message to Forwarding Server %s", IP2String(tempbuff, network.dns[network.currentDNS]));
					logDNSMess(req, logBuff, 1);
					addRRNone(req);
					req->dnsp->header.rcode = RCODE_SERVERFAIL;
				}
			...
			else
			{
				if (verbatim || cfig.dnsLogLevel >= 2)
				{
					sprintf(logBuff, "%s forwarded to Forwarding Server %s", strquery(req), IP2String(tempbuff, network.dns[network.currentDNS]));
					logDNSMess(req, logBuff, 2);
				}
			}
		}
...
{
	//printf("Adding %s=%s \n", hostname,IP2String(tempbuff, ip));
	//memset(&lump, 0, sizeof(data71));
...
*/
		...
		//printf("Added %s=%s\n", IP2String(tempbuff, ip), hostname);
	}
...
void listCache()
{
	...
	while (p != dnsCache[currentInd].end())
	{
		...
		if (cache->hostname)
			...
			sprintf(logBuff, "%s=%s", cache->mapname, IP2String(tempbuff, cache->ip));
...
data7 *findEntry(MYBYTE ind, char *key, MYBYTE entryType)
{
	...
	while (it != dnsCache[ind].end() && it->second && !strcasecmp(it->second->mapname, key))
	{
...
data7 *findEntry(MYBYTE ind, char *key)
{
	...
	if (it != dnsCache[ind].end() && it->second)
		return it->second;
...
MYDWORD getZone(MYBYTE updateCache, char *zone)
{
	...
	if (connect(req.sock, (sockaddr*)&req.remote, req.sockLen) >= 0)
	{
		...
		{
			closesocket(req.sock);
			sprintf(logBuff, "Failed to contact Primary Server %s, Zone Transfer Failed", IP2String(tempbuff, req.remote.sin_addr.s_addr));
			logDNSMess(logBuff, 1);
			return 0;
		}
...
bool getSecondary()
{
	...
	{
//		errno = WSAGetLastError();
//		sprintf(logBuff, "Failed to connect to Secondary Server %s, WSAError %u", IP2String(tempbuff, req.remote.sin_addr.s_addr), errno);
//		logDNSMess(logBuff, 1);
		closesocket(req.sock);
		return false;
	}
	...
	if (dnsService)
	{
		...
		for (int i = 0; i < MAX_COND_FORW && cfig.dnsRoutes[i].dns[0]; i++)
		{
			...
			if (!cfig.dnsRoutes[i].dns[1])
				sprintf(logBuff, "Conditional Forwarder: %s for %s", IP2String(tempbuff, cfig.dnsRoutes[i].dns[0]), cfig.dnsRoutes[i].zone);
			else
				sprintf(logBuff, "Conditional Forwarder: %s, %s for %s", IP2String(temp, cfig.dnsRoutes[i].dns[0]), IP2String(tempbuff, cfig.dnsRoutes[i].dns[1]), cfig.dnsRoutes[i].zone);
		...
		{
			sprintf(logBuff, "Default Forwarding Server: %s", IP2String(tempbuff, network.dns[i]));
			logDNSMess(logBuff, 1);
		}
		...
		if (dnsService)
		{
			...
			for (int j = 0; j < MAX_SERVERS && network.listenServers[j]; j++)
			{
				...
				if (nRet == SOCKET_ERROR)
				{
					bindfailed = true;
					closesocket(network.dnsUdpConn[i].sock);
					sprintf(logBuff, "Warning: %s UDP Port 53 already in use", IP2String(tempbuff, network.listenServers[j]));
					logDNSMess(logBuff, 1);
					continue;
				}
			...
			for (int j = 0; j < MAX_SERVERS && network.listenServers[j]; j++)
			{
				...
				else
				{
					...
					if (nRet == SOCKET_ERROR)
					{
						bindfailed = true;
						closesocket(network.dnsTcpConn[i].sock);
						sprintf(logBuff, "Warning: %s TCP Port 53 already in use", IP2String(tempbuff, network.listenServers[j]));
						logDNSMess(logBuff, 1);
					}
					else
					{
...
{
	MYWORD dataSize = 3 + sizeof(data7) + strlen(lump->mapname);
